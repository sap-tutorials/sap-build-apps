...
When first running the app, we check if there is a cart for the current user. If yes, we take that order ID; if not, we create a new order with the status **CART**.
...
2. Click on an empty place on the UI canvas, and then click **Add logic to HOME PAGE** link at the bottom of the page. This opens the page's logic canvas.
...
3. To the **Page focused** event, add flow functions and then connect them so they look like this.
    ...
    >**IMPORTANT:** A flow function can have multiple outputs (e.g., the first for success and the second for failure). Make sure to attach the flow functions to each other using the indicated outputs.

    >![Outputs](images/Outputs.png)

4. The first **If condition** checks if we don't have a cart. If we DO have a cart, we do nothing. Otherwise, we proceed with retrieving or creating a cart.
...
5. We need to make sure the system variables such as email address of the current user are already available to use in the next step. 
    
    We have a **delay** function between the first **If condition** flow function and the **Get Record Collection** flow function. This **delay** function
    ensures that all the system variables are present and available to use.

6. If we do not have a cart, we use the **Get record collection** to retrieve the orders for the current user, filtering for any with the status **CART**.
    ...
    Make sure to set **ALL** for **Matching ALL of these conditions**.  

7. We now have to see if we were able to retrieve any orders for the current user that is a cart.
...
8. The second **If condition** determines if there is an existing cart.
    ...
    >The formula sets sets the customer to the current user, and sets the status of the record to **CART**, meaning it represents a cart and not an actually order yet.
    ...
    Out of the top output of the **Create record** flow function, we now have to set the app variable to the ID of the new record we created.
...
4. Select the **Create Record** function, and in the **Properties** tab on the right side, set the following:
    ...
    - **Record** to the following formula.
        ...
        >The formula will appear red with a warning, because the ID field is mandatory but the formula editor does not know that the ID will be auto-generated. Just click **Save**.
...
5. Now that we retrieved the new list of items, we have to update our data variable.
    ...
    - **Data variable name** to **OrderItems1**.
...
5. Click **Save**, and then **Save** again (upper right).
...
Go back to the home page and click the first product, then on the product details page, change the quantity to `7`  and click **Add to Cart**.
