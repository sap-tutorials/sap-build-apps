...
In this tutorial, we will create a new page to display the processes we have triggered, and let the user click a button for each to check if its status was updated.

>We will use the process flow functions that come with SAP Build Apps, which this task easy. You can see also use a wide range of APIs to interact with processes, and these are described in the [SAP Business Accelerator Hub](https://api.sap.com/package/SAPProcessAutomation/all). Select the API you are interested in, and then click **View the API Reference**.
...
4. Copy the following text:
    ...
    Select **Page Layout** in the tree view at the bottom right, and press **Control-V** (**Command-V** on Mac).
...
3. Under **Processes**, drag in a **Retrieve Instances** flow function and connect it to the **Page Mounted** event.
    ...
    With the flow function selected and in the **Properties** tab, set **Process** to Order Processing**.

    ![Open page](retrieve3.png)


    Set **startedBy** to **Data and Variables > System Variables >     Currently logged in user >  email**.
